...
// DEFINE EDGE
Edge = function(node1, node2, multi, label, edge_type) {
    this.node1 = node1;
    this.node2 = node2;
    this.multi = multi || 1;
    this.label = label || '{}';
    this.edge_type = edge_type || default_edge_type;
};
...
Edge.prototype = {
	...
	// set style
	if ( this.edge_type === "edge" ) { ctx.setLineDash([]); }
	if ( this.edge_type === "nonedge" ) { ctx.setLineDash([5,10]); }
	// for background_edge;
	if ( this.edge_type === "background_edge" ) { 
...
Controller = function() {
    ...
    return {
        ...
        keypress: function(e) {
            ...
            if (String.fromCharCode(e.charCode) === 'e' ) {
                default_edge_type = "edge";
            }
            if (String.fromCharCode(e.charCode) === 'n' ) {
                default_edge_type = "nonedge";
            }
            if (!LIVE) draw();
        },
        mouseleave: function() {
...
function import_from_JSON(JSONdata) {
    ...
    for (i = 0; i < data.edges.length; i += 1) {
        edge_list.push(new Edge(dict[data.edges[i][0]],
				dict[data.edges[i][1]], 1, data.edges[i][2],data.edges[i][3]));
    }
...
function export_sage() {
    ...
    data.edges = edge_list.map(function(e) {
        return [e.node1.label, e.node2.label, e.label, e.edge_type];
    });
    data.pos = {};
    for (i = 0; i < nodes.length; i++) {
        pos = nodes[i].get_pos();
...
function create_controls(div) {
    ...
    add_slider("Edge Type: Nonedge/Edge", 1, tweaks, 0, 1, function(newval) {
	if ( newval == 1 ) { default_edge_type = "edge"; }
	if ( newval == 0 ) { default_edge_type = "nonedge"; }
    });
...
function display_graph() {
	...
	for ( j = i+1; j < nodes.length; j += 1){
	    temp_edge = new Edge(nodes[i],nodes[j],1,'{}',"background_edge");
	    temp_edge.display();
	}
    }
    for (i = 0; i < edge_list.length; i += 1) {
